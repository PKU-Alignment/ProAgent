...
class OvercookedGridworld(object):
    ...
    def get_state_transition(self, state, joint_action):
        ...
        sparse_reward, shaped_reward = self.resolve_interacts(new_state, joint_action) ## I add the ml_actions feature through this fun
        ...
        for i, (player, action) in enumerate(zip(new_state.players, joint_action)):
            ...
            if terrain_type == 'X':
                if player.has_object() and not new_state.has_object(i_pos):
                    ml_actions[i] = f'place_{player.held_object.name}_on_counter'
                    ...
                    ml_actions[i] = f'pickup_{player.held_object.name}'
                ...
                if player.get_object().name == 'dish' and new_state.has_object(i_pos):
                    ...
                    if num_items == self.num_items_for_soup and cook_time >= self.soup_cooking_time:
                        ...
                        ml_actions[i] = 'fill_dish_with_soup'
                    ...
                    ml_actions[i] = f'put_{item_type}_in_pot'
