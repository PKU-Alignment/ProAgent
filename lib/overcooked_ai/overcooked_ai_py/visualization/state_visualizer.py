...
class StateVisualizer:
    ...
    def default_hud_data(state, **kwargs):
        ...
        #todo
        result={"timestep": state.timestep,"all_orders":[{'ingredients': ('onion', 'onion', 'onion')}], "bonus_orders":[]}
    ...
    def _render_objects(self, surface, objects, grid):
        def render_soup(surface, obj, grid):
            ...
            frame_name = StateVisualizer._soup_frame_name('_'.join([obj.state[0]]*obj.state[1]), soup_status)
    ...
    def _render_cooking_timers(self, surface, objects, grid):
        for key, obj in objects.items():
            ...
            if obj.name == "soup" and grid[y_pos][x_pos] == POT:
                ...
                if obj.state[2] != 0 and (obj.state[2] <= 20 or self.show_timer_when_cooked):
                    #todo check 
